    ...
    try {
      if (actionType === "lock" || actionType === "unlock") {
        ...
        setPartners(
          partners.map((partner) =>
            partner.id === selectedPartner.id
              ? { ...partner, isLocked: actionType === "lock" }
              : partner
          )
        );
        ...
        const response = await axios.delete(
          `${config.API_BASE_URL}/api/v1/users/partners/${selectedPartner.id}/soft`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        const response = await axios.delete(
          `${config.API_BASE_URL}/api/v1/users/partners/${selectedPartner.id}/hard`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          ...
          <div className="absolute top-0 left-0 h-full w-full flex items-center justify-center">
            ...
            >
              ...
              <path
                ...
                fill="currentColor"
                d="M4 12a8 8 0 018-8v4l-3 3-1-1-3 3a8 8 0 010-8z"
              ></path>
            </svg>
          </div>
        </div>
          ...
          <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            ...
            <div className="mt-6 flex justify-center">
              ...
              >
                {[
                  ...Array(
                    Math.ceil(filteredPartners.length / partnersPerPage)
                  ).keys(),
                ].map((number) => (
                  <button
                    key={number}
                    onClick={() => paginate(number + 1)}
                    className={`relative inline-flex items-center px-4 py-2 border border-gray-300 bg-blue text-sm font-medium hover:bg-blue-500 hover:text-white transition duration-300 ${
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg w-1/3">
          ...
          <p className="mb-6">
            ...
            {actionType === "hardDelete"
              ? "permanently delete this partner? This action will unassign all the bookings done by customers."
              : actionType === "softDelete"
              ? "soft delete this partner? You can restore it later if needed."
              : `${actionType} this partner?`}
          </p>
