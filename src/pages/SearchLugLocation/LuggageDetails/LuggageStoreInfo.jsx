    ...
    const fetchReviews = async () => {
      try {
        const response = await fetch(
          `${config.API_BASE_URL}/api/v1/reviews/location/${id}`
        );
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-8 mb-8">
      ...
      <p className="mb-4 text-lg">
        ...
        <strong>{averageRating.toFixed(1)}</strong> out of 5 stars (
        {reviewCount}+ reviews)
      </p>
      <p className="mb-4 flex items-center text-lg">
        <FontAwesomeIcon icon={faWifi} className="text-[#1A73A7] mr-3" />
        <strong>Services:</strong>{" "}
        <span className="ml-2">One of the best Luggage Service Providers</span>
      </p>
      <p className="flex items-center mb-6 text-lg">
        <FontAwesomeIcon icon={faClock} className="text-[#1A73A7] mr-3" />
        <strong>Open Time:</strong>{" "}
        <span className="ml-2">{formatTime(openTime)}</span>
        <FontAwesomeIcon icon={faClock} className="ps-4 text-[#1A73A7]" />
        <strong className="ps-2">Close Time:</strong>{" "}
        <span className="ml-2">{formatTime(closeTime)}</span>
      </p>
      ...
      <div className="mt-12">
        ...
        <div className="bg-gray-100 p-6 rounded-lg shadow-md text-center">
          ...
          <p className="text-gray-700">
            Based on {reviewCount}+ anonymous customer ratings.
          </p>
        </div>
      </div>
    </div>
