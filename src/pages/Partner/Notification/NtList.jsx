...
function AllNotificationPartner() {
  ...
  const [activeId, setId] = useState(state?.ntId || -1);
  const handleId = (e) => {
    setId(e);
    console.log(e);
  };
  ...
  const fetchNoti = async (p) => {
    ...
    const response = await axios.get(
      `${url}/api/v1/notifications/partner?page=${p}&limit=25`,
      {
        headers: {
  ...
  useEffect(() => {
    if (state?.ntId) {
      if (list.length > 0) {
        setId(list.findIndex((f) => f?._id === state?.ntId));
      }
    }
  }, [state, list]);
...
export default AllNotificationPartner;
