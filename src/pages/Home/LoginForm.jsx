...
const LoginForm = ({ loginType, onClose }) => {
  ...
  const handleForgotPasswordSubmit = async (data) => {
    setIsLoading(true); // Set loading to true when form is submitted
    const endpoint = `${config.API_BASE_URL}/api/v1/users/reset-password-request`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showForgotPassword ? (
          <>
            <h2 className="text-center mb-4 text-3xl font-bold text-blue-700">
              Forgot Password
            </h2>
            <form onSubmit={handleSubmit(handleForgotPasswordSubmit)}>
              ...
              {rstMsg && (
                <p className="text-green-500 mb-4 py-2 break-words">{rstMsg}</p>
              )}
              <div className="mb-4">
                <button
                  className="w-full bg-blue-600 hover:bg-blue-800 text-white py-2 px-4 rounded transition duration-300 ease-in-out transform hover:scale-105"
                  type="submit"
                >
                  {isLoading ? (
