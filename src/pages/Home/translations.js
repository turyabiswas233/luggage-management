const translations = {
  en: {
    header: {
      home: "Home",
      guideline: "Guideline",
      urlokerKeys: "Urloker Keys",
      chargingLocation: "Charging Spot",
      login: "Login",
      language: "English",
    },
    heroSection: {
      title: "Freedom in Every Journey",
      subtitle: "Looking For Luggage Services? We Are Here...",
      searchPlaceholder: "Enter your location",
      searchButton: "Search",
      findLocationsButton: "Find Closest Locations",
    },
    faqSection: {
      title: "Have Any Questions? We Are Here To Answer You",
      faqs: [
        {
          question: "How does the Urloker luggage storage system work?",
          answer:
            "UrLoker provides collection and delivery services. Click on a location or book a van in the same way that you would book a doctor's appointment through the website. Book a flight by telephone, specifying the location, date and time. As for the dropping out, you will have to put your bag in a locker and as for pick up, the staff will take it and keep it safely.",
        },
        {
          question: "What things can I store?",
          answer: `
            <li>Luggage: franchises of suitcases, big bags duffle</li>
            <li>Personal items: Women's purses, Computer Case</li>
            <li>Bulky Items: Sporting equipment, Child params</li>
            `,
        },
        {
          question: "Are there size limits?",
          answer:
            "We welcome items of all sizes! However, for extra-large or bulky items like surfboards, skis, or bicycles, we kindly ask for prior approval to ensure our partners can accommodate your needs. Don’t worry—we’ll handle the coordination and keep you informed.",
        },
        {
          question: "Do larger bags cost more?",
          answer:
            "No. We won't charge you more for your bigger luggeages. So, enjoy the same rates, exclusively with Urloker.",
        },
        {
          question: "Is hourly luggage storage available at Urloker?",
          answer:
            "We offer a flat 24-hour rate, ensuring you get the same great price whether you store your items for just a few hours or the entire day.",
        },
        {
          question:
            "Can I book luggage storage for the whole day with a single booking?",
          answer: `Yes, you can easily book luggage storage for the entire day with just one booking, ensuring a hassle-free experience.`,
        },
        {
          question: "Looking for luggage lockers nearby?",
          answer:
            "We partner with trusted businesses that provide secure storage areas for your belongings, offering the same safety and convenience as traditional lockers—without the hassle of availability or size constraints.",
        },
        {
          question: "Need to change or cancel your booking?",
          answer:
            "We understand that plans can change! Easily modify or cancel your booking right from the details page in our app. Adjust dates, drop-off and pick-up times, or the number of bags with just a few taps.",
        },
        {
          question: "Can I get a refund if I cancel?",
          answer:
            "Absolutely! You can cancel your booking anytime before check-in for a full refund, and best of all—no cancellation fees!",
        },
        {
          question: "What security measures are in place?",
          answer: `
            <li>Super Locks</li>
            <li>CCTV</li>
            <li>Staff trained</li>
          `,
        },
        {
          question: "what kind of service Urloker Luggage Storage provide?",
          answer:
            "Urloker Luggage Storage provides secure and monitored luggage storage solutions with flexible options for all bag sizes. They also offer key storage services for Airbnb hosts and guests, with easy online and QR code booking at partner locations. Additionally, Urloker provides 24/7 customer support, real-time notifications, and optional insurance for added peace of mind.",
        },
      ],
    },
    reviewSection: {
      title: "Our customers say it best",
      reviews: [
        {
          name: "Mason Doe",
          position: "Client",
          text: "The luggage service provided by this company is exceptional. Their attention to detail and commitment to quality have greatly enhanced our travel experience. Highly recommended!",
          rating: 5,
        },
        {
          name: "Alisa Decros",
          position: "Client",
          text: "I have been thoroughly impressed with the luggage services. From prompt delivery to the pristine condition of the luggage, everything was perfect. Their professionalism is unmatched.",
          rating: 5,
        },
        {
          name: "Jac Lorry",
          position: "Client",
          text: "Thank you for providing such a fantastic customer experience. The luggage is well-made and the team's support has been invaluable. I'll definitely be a repeat customer.",
          rating: 5,
        },
        {
          name: "Jackson",
          position: "Client",
          text: "Outstanding service! The team goes above and beyond to ensure customer satisfaction. The luggage quality is top-notch and their support is always there when you need it.",
          rating: 5,
        },
        {
          name: "Robert",
          position: "Client",
          text: "Excellent service! The team goes above and beyond to ensure customer satisfaction. The luggage quality is top-notch and their support is always there when you need it.",
          rating: 5,
        },
      ],
    },
    blogSection: {
      title: "Tourist Spots",
      subtitle: "Explore Our Destinations",
      posts: [
        {
          title: "Sydney Harbour Bridge",
          author: "Admin",
          comments: 20,
          views: 11,
          imageUrl: "furniture",
          excerpt:
            "The Sydney Harbour Bridge, an iconic steel arch, connects Sydney's CBD to the North Shore, offering stunning harbour views. Opened in 1932, it's a major transportation link and a symbol of Australia's engineering prowess",
          detailsUrl: "sydney-harbour-bridge",
        },
        {
          title: "Taronga Zoo Sydney",
          author: "Admin",
          comments: 21,
          views: 15,
          imageUrl: "hospital",
          excerpt:
            "Situated on Sydney Harbour, this zoo offers diverse wildlife exhibits and stunning views of the city skyline.",
          detailsUrl: "tarongo-zoo-sydney",
        },
        {
          title: "Great Ocean Road",
          author: "Admin",
          comments: 22,
          views: 20,
          imageUrl: "luggage",
          excerpt:
            "A scenic coastal drive near Melbourne, featuring stunning ocean views, the Twelve Apostles, and charming seaside towns.",
          detailsUrl: "great-ocean-road",
        },
      ],
    },

    whyChooseUs: {
      title: "Why Choose Us?",
      points: [
        {
          title: "Security Guaranteed",
          description: `"Your bags are safe with us!" We ensure a 100% secure environment for your luggage, featuring comprehensive coverage and advanced surveillance systems. Enjoy peace of mind while you explore!`,
        },
        {
          title: "One Size Fits All",
          description:
            "Whether it's a suitcase or a backpack, our flat rate covers all. No hidden fees, no size restrictions!",
        },
        {
          title: "All-Day Access",
          description:
            "Secure your luggage once and access it anytime throughout the day. Your belongings are safe and accessible whenever you need them.",
        },
        {
          title: "Hassle-Free Booking",
          description:
            "Experience our streamlined booking process! Easy steps, quick confirmation, and unbeatable prices all in one platform.",
        },
        {
          title: "24/7 Customer Support",
          description:
            "Our dedicated team is available around the clock to assist you. Got questions? Our chatbot and customer service are ready to help anytime!",
        },
      ],
    },
    footer: {
      company: "Company",
      about: "About",
      services: "Services",

      blog: "Blog",
      support: "Support",
      faq: "FAQ",
      privacyPolicy: "Privacy Policy",
      terms: "Terms & Conditions",

      contactInfo: "Contact Info",
      address: "Address: 1 William St, Hawthorn, Victoria 3122, Australia",
      email: "Email: admin@urloker.com",
      phone: "Phone: +01 234 567 89",
      followUs: "Follow Us",
      companyDescription:
        "Urloker offers secure and convenient luggage storage solutions.",
      copyright: "{year} Urloker. Designed By EasytechInnovation",
    },
    luggageStorageLocations: {
      title: "Our Top Luggage Storage Locations",
      searchButton: "Search Location",
    },
    howItWorks: {
      title: "How it Works: A Quick Guide",
      steps: {
        book: {
          title: "Book",
          description:
            "Find your desired location and book effortlessly with unbeatable prices.",
        },
        lock: {
          title: "Lock",
          description:
            "Present your booking confirmation and leave your bags securely with us.",
        },
        enjoy: {
          title: "Enjoy",
          description:
            "Explore freely, and when ready, retrieve your belongings without any hassle.",
        },
      },
      video: {
        title: "How It Works Video",
        altText: "Guideline",
      },
    },
    about: {
      title: "About Us",
      info: [
        `At Urloker, we provide the best luggage storage solutions in
          Australia, offering a seamless, secure, and affordable experience for
          travellers. Whether you're exploring a new city or need a safe place
          to store your belongings, our hassle-free booking process, unbeatable
          rates, and growing network of partner locations make it easy to store
          your luggage with confidence.`,
        ` Customers can book storage directly through our website or by scanning
          a QR code at any of our partner stores, allowing for maximum
          flexibility and convenience. Our advanced security measures, including
          surveillance systems, ensure that your bags are safe and accessible
          whenever you need them. Our No Rush Retrieval policy gives you freedom
          to Store your luggage and retrieve it at your convenience—no time
          limits, no pressure, just freedom to explore at your own pace.`,
        `Urloker is committed to provide best customer service. Our dedicated
          chatbot is available 24/7 to help resolve any customer service
          inquiries quickly and efficiently, ensuring a smooth experience from
          start to finish.`,
        `As the leading luggage storage service provider in Australia, our
          slogan is Freedom in Every Journey which depicts our mission to
          provide every customer with peace of mind, knowing their belongings
          are in safe hands while they enjoy the freedom of exploring the city.`,
      ],
    },
    cityInfo: {
      title: "Luggage Storage in Melbourne - Simple, Safe and Price-wise",
      desc: `Luggage storage in Melbourne has never been easier or more convenient than with UrLoker. We know the struggle of having to carry heavy bags when out exploring the marvelous city of Melbourne. Whether it is your first time visiting Melbourne as a tourist, a business traveler on the go or even someone who has a layover in Melbourne Airport, our easy location and secured storage guarantee a safe and pleasant experience throughout the whole trip.\\break At UrLoker, we are on a mission of ensuring that every resident and visitor to Melbourne has easy and reliable access to cheap, safe and effective luggage storage facilities, which will enable them to move around the city freely. Most airport buggage lockers & storage facilities are located inside Melbourne airports, Southern cross station, and flinders street station so there is no way the city can be so big that we don't have a storage close to you. If you are looking for a range of services whether short term or long term, what you need is at UrLoker.`,
      otherInfo: [
        {
          title: "Luggage Storage Melbourne Airport",
          desc: "If you are flying to or from Melbourne Airport, you can use the luggage storage facilities prior or after your flight to not stress or worry about your bags. If you are a traveler with a long layover or just several hours before the next flight, use our Melbourne Airport luggage lockers that have such a perfect option.",
        },
        {
          title: "Luggage Storage in Melbourne CBD",
          desc: "For inner city visitors, as you tour the central business district, you can utilize our luggage storage services spread over the Melbourne CBD. With our lockers strategically placed, you can dump your travel bags and come out to see all the famous sites, shop and eat with no back baggage.",
        },
        {
          title: "Southern Cross Station Luggage Storage",
          desc: "Southern Cross station is one of the main terminals in Melbourne and therefore perfect if you have luggage to be kept. While you are waiting to take your train, bus or shuttle, you can easily leave your bags with us and enjoy the Southern Cross Station luggage storage facility.",
        },
        {
          title: "Flinders Street Station Luggage Storage",
          desc: "Flinders Street Station is a busy operational hub and one of the most recognizable buildings spiral structure to view. Our storage facilities here offer a safe way to stow your bags while visiting nearby sites such as Federation Square, National Gallery of Victoria, or Yarra River.",
        },
      ],
      whyBest: [
        {
          title: "Best Locations",
          desc: "We have strategically placed our luggage storage lockers in prime areas of Melbourne such as the international airport, Southern Cross Station, and Flinders Street Station to diversify clientele access no matter where they are within the city.",
        },
        {
          title: "Safety and Trustworthieness",
          desc: "We know that there is always a safety concern when it comes to the things you carry with you. It also has secure lockers for the clients equipped with competent locking systems and 24/7 video surveillance, therefore, you will be free to move around without worrying about your luggage.",
        },
        {
          title: "Easy to Store Your Luggage",
          desc: "If you want to leave your bags for a couple of hours or a full day or more, we have flexible storage solutions which are meant to cover your needs. If you have changed some of your plans, you can add extra time to your storage without worrying that you're missing out on much of your time in Melbourne.",
        },
        {
          title: "No High Prices",
          desc: "It is a good thing to know that at UrLoker, there is no experience where luggage storage is offered at exorbitant fees. With our affordable rates, you are sure that you have the best possible service for your money without worrying about high storage costs.",
        },
        {
          title: "No Long Wait for Admission",
          desc: "Booking of the lockers is very fast and does not waste time. A locker can be secured in advance to be used on a later date or otherwise, on reaching the venue, one can get a locker on the spot. To use our system, you do not need to register, you need to put your luggage through our luggage storage in a few clicks.",
        },
        {
          title:
            "Avoid Carrying Heavy Luggage While Traveling Around the City of Melbourne",
          desc: "Melbourne is a city that is full of cultural, historical, and other activities. If it is for you to catch up with the regular tourist spots like Federation Square, walk through the laneways of the CBD or simply take a walk by the Yarra River, UrLoker has got you covered on all whereas you do not have to lug around your bags and walk around the city.<br />Thanks to our network of storage facilities which include important places like the Melbourne Airport, Southern Cross Station and Flinders Street Station, you don't have to think about your bags while having fun in Melbourne.",
        },
        {
          title: "Make Your Luggage Storage Booking Today with UrLoker",
          desc: "Style your Melbourne trip without heavy bags or luggage restricting you. UrLoker enables you to keep your luggage safe and sound while sight-seeing, thanks to its safe and economical luggage storage services. Reserve your luggage storage services now and discover Melbourne hassle free!",
        },
      ],
    },
    demo: {
      title: "Hassle Free Luggage Storage, Book in Seconds",
      list: [
        {
          title: "Security Guaranteed",
          desc: "$1000 protection",
        },
        {
          title: "Size Doesn't Matter",
          desc: "Flat Rate for All Bag Size No Need to Pay Extra",
        },
        {
          title: "No Hourly Fees",
          desc: "Enjoy full Day Flexibility with a Fixed Price Unbeatable Prices",
        },
        {
          title: "24/7 Support",
          desc: "We are Always Here for You",
        },
      ],
      end: "Enjoy Freedom in Every Journey with Urloker",
    },
  },
  es: {
    header: {
      home: "Inicio",
      guideline: "Guía",
      urlokerKeys: "Llaves Urloker",
      chargingLocation: "Punti di ricarica",
      login: "Iniciar sesión",
      language: "Idioma",
    },
    luggageStorageLocations: {
      title: "Nuestras Principales Ubicaciones de Almacenamiento de Equipaje",
      searchButton: "Buscar Ubicación",
    },
    howItWorks: {
      title: "Cómo Funciona: Una Guía Rápida",
      steps: {
        book: {
          title: "Reserva",
          description:
            "Encuentra tu ubicación deseada y reserva sin esfuerzo con precios inmejorables.",
        },
        lock: {
          title: "Cierra",
          description:
            "Presenta tu confirmación de reserva y deja tus maletas con nosotros de manera segura.",
        },
        enjoy: {
          title: "Disfruta",
          description:
            "Explora libremente, y cuando estés listo, recupera tus pertenencias sin ningún problema.",
        },
      },
      video: {
        title: "Video de Cómo Funciona",
        altText: "Guía",
      },
    },
    footer: {
      company: "Compañía",
      about: "Acerca de",
      services: "Servicios",

      blog: "Blog",
      support: "Apoyo",
      faq: "FAQ",
      privacyPolicy: "Política de Privacidad",
      terms: "Términos y Condiciones",

      contactInfo: "Información de Contacto",
      address: "Dirección: 1 William St, Hawthorn, Victoria 3122, Australia",
      email: "Correo electrónico: admin@urloker.com",
      phone: "Teléfono: +01 234 567 89",
      followUs: "Síguenos",
      companyDescription:
        "Urloker ofrece soluciones de almacenamiento de equipaje seguras y convenientes para su hogar y oficina.",
      copyright: "{year} Urloker. Diseñado Por EasytechInnovation",
    },

    heroSection: {
      title: "Libertad en Cada Viaje",
      subtitle: "¿Buscas servicios de equipaje? Estamos aquí...",
      searchPlaceholder: "Ingrese su ubicación",
      searchButton: "Buscar",
      findLocationsButton: "Encontrar ubicaciones más cercanas",
    },
    faqSection: {
      title: "¿Tienes alguna pregunta? Estamos aquí para responderte",
      faqs: [
        {
          question: "¿Qué puedo almacenar con Urloker?",
          answer:
            "Desde maletas y mochilas hasta bolsas de gimnasio y más allá, nuestro servicio es como tener una solución de almacenamiento personal donde quiera que estés. Ya sea que estés entre Airbnbs o saliendo de noche, puedes almacenar más que solo tu equipaje con nosotros.",
        },
        {
          question: "¿Hay límites de tamaño?",
          answer:
            "¡Aceptamos artículos de todos los tamaños! Sin embargo, para artículos extragrandes o voluminosos como tablas de surf, esquís o bicicletas, pedimos aprobación previa para asegurar que nuestros socios puedan acomodar tus necesidades. No te preocupes, nos encargaremos de la coordinación y te mantendremos informado.",
        },
        {
          question: "¿Las bolsas más grandes cuestan más?",
          answer:
            "No. No le cobraremos más por sus maletas más grandes. Por lo tanto, disfrute de las mismas tarifas, exclusivamente con Urloker.",
        },
        {
          question:
            "¿Está disponible el almacenamiento de equipaje por horas en Urloker?",
          answer:
            "Ofrecemos una tarifa plana por 24 horas, asegurando que obtienes el mismo gran precio, ya sea que guardes tus artículos por solo unas pocas horas o todo el día.",
        },
        {
          question: "¿Buscas taquillas de equipaje cercanas?",
          answer:
            "Nos asociamos con negocios de confianza que proporcionan áreas de almacenamiento seguras para tus pertenencias, ofreciendo la misma seguridad y conveniencia que las taquillas tradicionales, sin las molestias de disponibilidad o restricciones de tamaño.",
        },
        {
          question: "¿Necesitas cambiar o cancelar tu reserva?",
          answer:
            "¡Entendemos que los planes pueden cambiar! Modifica o cancela fácilmente tu reserva desde la página de detalles en nuestra app. Ajusta fechas, horas de entrega y recogida, o la cantidad de bolsas con solo unos toques.",
        },
        {
          question: "¿Puedo obtener un reembolso si cancelo?",
          answer:
            "¡Absolutamente! Puedes cancelar tu reserva en cualquier momento antes del check-in para obtener un reembolso completo, y lo mejor de todo: ¡sin tarifas de cancelación!",
        },
      ],
    },
    reviewSection: {
      title: "Nuestros clientes lo dicen mejor",
      reviews: [
        {
          name: "Mason Doe",
          position: "Cliente",
          text: "El servicio de equipaje proporcionado por esta empresa es excepcional. Su atención al detalle y compromiso con la calidad han mejorado en gran medida nuestra experiencia de viaje. ¡Altamente recomendado!",
          rating: 5,
        },
        {
          name: "Alisa Decros",
          position: "Cliente",
          text: "He quedado muy impresionado con los servicios de equipaje. Desde la entrega rápida hasta el estado impecable del equipaje, todo fue perfecto. Su profesionalismo es inigualable.",
          rating: 5,
        },
        {
          name: "Jac Lorry",
          position: "Cliente",
          text: "Gracias por brindar una experiencia al cliente tan fantástica. El equipaje está bien hecho y el apoyo del equipo ha sido invaluable. Definitivamente seré un cliente habitual.",
          rating: 5,
        },
        {
          name: "Jackson",
          position: "Cliente",
          text: "¡Servicio excepcional! El equipo va más allá para asegurar la satisfacción del cliente. La calidad del equipaje es de primera y su soporte siempre está allí cuando lo necesitas.",
          rating: 5,
        },
        {
          name: "Robert",
          position: "Cliente",
          text: "¡Excelente servicio! El equipo va más allá para asegurar la satisfacción del cliente. La calidad del equipaje es de primera y su soporte siempre está allí cuando lo necesitas.",
          rating: 5,
        },
      ],
    },

    whyChooseUs: {
      title: "¿Por Qué Elegirnos?",
      points: [
        {
          title: "Seguridad Garantizada",
          description: `"¡Tus maletas están seguras con nosotros!" Aseguramos un entorno 100% seguro para tu equipaje, con cobertura integral y sistemas avanzados de vigilancia. ¡Disfruta de la tranquilidad mientras exploras!`,
        },
        {
          title: "Un Tamaño para Todos",
          description:
            "Ya sea una maleta o una mochila, nuestra tarifa plana lo cubre todo. ¡Sin tarifas ocultas, sin restricciones de tamaño!",
        },
        {
          title: "Acceso Todo el Día",
          description:
            "Asegura tu equipaje una vez y accede a él en cualquier momento del día. Tus pertenencias están seguras y accesibles cuando las necesites.",
        },
        {
          title: "Reserva Sin Complicaciones",
          description:
            "¡Experimenta nuestro proceso de reserva simplificado! Pasos sencillos, confirmación rápida y precios inmejorables, todo en una plataforma.",
        },
        {
          title: "Soporte al Cliente 24/7",
          description:
            "Nuestro equipo dedicado está disponible las 24 horas para asistirte. ¿Tienes preguntas? Nuestro chatbot y servicio de atención al cliente están listos para ayudar en cualquier momento.",
        },
      ],
    },
    blogSection: {
      title: "Lugares turísticos",
      subtitle: "Explora Nuestros Destinos",
      posts: [
        {
          title: "Puente de la Bahía de Sídney",
          author: "Admin",
          comments: 20,
          views: 11,
          imageUrl: "furniture",
          excerpt:
            "Descubre cómo nuestros muebles y herramientas meticulosamente limpios mejoran tu experiencia de viaje. Aprende por qué somos confiados por viajeros de todo el mundo.",
          detailsUrl: "blog-details.html",
        },
        {
          title:
            "Nuestro Compromiso con el Equipaje Hospitalario para Causas Sociales",
          author: "Admin",
          comments: 21,
          views: 15,
          imageUrl: "hospital",
          excerpt:
            "Lee sobre nuestras iniciativas para proporcionar equipaje especializado a hospitales. Mira cómo contribuimos a causas sociales a través de nuestros servicios.",
          detailsUrl: "blog-details.html",
        },
        {
          title: "Excelencia en Servicios de Equipaje - Trabajamos para Ti",
          author: "Admin",
          comments: 22,
          views: 20,
          imageUrl: "luggage",
          excerpt:
            "Aprende más sobre nuestra dedicación a ofrecer los mejores servicios de equipaje. Priorizamos tus necesidades para asegurar una experiencia de viaje sin contratiempos.",
          detailsUrl: "blog-details.html",
        },
      ],
    },
    about: {
      title: "Sobre nosotros",
      info: [
        `En Urloker, ofrecemos las mejores soluciones de almacenamiento de equipaje en
Australia, ofreciendo una experiencia sencilla, segura y asequible para
los viajeros. Ya sea que esté explorando una nueva ciudad o necesite un lugar seguro
para guardar sus pertenencias, nuestro proceso de reserva sin complicaciones, tarifas
inmejorables y nuestra creciente red de ubicaciones asociadas hacen que sea fácil almacenar
su equipaje con confianza.`,
        ` Los clientes pueden reservar el almacenamiento directamente a través de nuestro sitio web o escaneando
un código QR en cualquiera de nuestras tiendas asociadas, lo que permite una máxima
flexibilidad y comodidad. Nuestras medidas de seguridad avanzadas, incluidos
sistemas de vigilancia, garantizan que su equipaje esté seguro y accesible
siempre que lo necesite. Nuestra política de recuperación sin prisas le brinda la libertad
de almacenar su equipaje y recuperarlo cuando lo desee, sin límites de tiempo, sin presiones, solo la libertad de explorar a su propio ritmo.`,
        `Urloker se compromete a brindar el mejor servicio al cliente. Nuestro chatbot
dedicado está disponible las 24 horas, los 7 días de la semana para ayudar a resolver cualquier consulta de servicio al cliente
de manera rápida y eficiente, lo que garantiza una experiencia fluida de principio a fin.`,
        `Como proveedor líder de servicios de almacenamiento de equipaje en Australia, nuestro
lema es Libertad en cada viaje, que refleja nuestra misión de
brindar a cada cliente tranquilidad, sabiendo que sus pertenencias
están en buenas manos mientras disfrutan de la libertad de explorar la ciudad.`,
      ],
    },
    demo: {
      title: "Hassle Free Luggage Storage, Book in Seconds",
      list: [
        {
          title: "Security Guaranteed",
          desc: "$1000 protection",
        },
        {
          title: "Size Doesn't Matter",
          desc: "Flat Rate for All Bag Size No Need to Pay Extra",
        },
        {
          title: "No Hourly Fees",
          desc: "Enjoy full Day Flexibility with a Fixed Price Unbeatable Prices",
        },
        {
          title: "24/7 Support",
          desc: "We are Always Here for You",
        },
      ],
      end: "Enjoy Freedom in Every Journey with Urloker",
    },
  },
  zh: {
    header: {
      home: "主页",
      guideline: "指南",
      urlokerKeys: "Urloker 密钥",
      chargingLocation: "充电点",
      login: "登录",
      language: "语言",
    },
    heroSection: {
      title: "每次旅程的自由",
      subtitle: "寻找行李服务？我们在这里...",
      searchPlaceholder: "输入您的位置",
      searchButton: "搜索",
      findLocationsButton: "查找最近的位置",
    },
    faqSection: {
      title: "有任何问题吗？我们在这里为您解答",
      faqs: [
        {
          question: "我可以在Urloker存放什么？",
          answer:
            "从行李箱和背包到健身包及其他，我们的服务就像是在任何地方都能提供个人存储解决方案。不论您是在Airbnb之间还是外出夜游，您都可以在我们这里存放的不仅仅是行李。",
        },
        {
          question: "是否有尺寸限制？",
          answer:
            "我们欢迎所有尺寸的物品！但是，对于超大型或笨重的物品，如冲浪板、滑雪板或自行车，我们要求提前批准，以确保我们的合作伙伴能够满足您的需求。别担心—我们会处理协调工作，并随时通知您。",
        },
        {
          question: "较大的行李价格更高吗？",
          answer:
            "不。我们不会为您的大件行李收取更多费用。因此，享受相同的价格，仅限 Urloker。",
        },
        {
          question: "Urloker是否提供按小时计算的行李存储？",
          answer:
            "我们提供24小时固定费率，无论您只存放几小时还是整天，我们都保证为您提供同样的优惠价格。",
        },
        {
          question: "在附近寻找行李寄存柜吗？",
          answer:
            "我们与值得信赖的商家合作，为您的物品提供安全的存储区域，提供与传统寄存柜相同的安全性和便利性—没有可用性或尺寸限制的烦恼。",
        },
        {
          question: "需要更改或取消您的预订？",
          answer:
            "我们理解计划可能会改变！您可以轻松地在我们的应用程序中从详情页修改或取消您的预订。只需点击几下，即可调整日期、投放和取件时间或行李数量。",
        },
        {
          question: "取消后我可以退款吗？",
          answer:
            "绝对可以！您可以在办理入住前随时取消预订，获得全额退款，最重要的是—没有取消费用！",
        },
      ],
    },

    whyChooseUs: {
      title: "为什么选择我们？",
      points: [
        {
          title: "安全保障",
          description: `"你的行李在我们这里很安全！" 我们为您的行李提供100%的安全环境，提供全面的覆盖和先进的监控系统。在您探索时享受安心！`,
        },
        {
          title: "一体化尺寸",
          description:
            "无论是手提箱还是背包，我们的一口价都能覆盖所有。没有隐藏费用，没有尺寸限制！",
        },
        {
          title: "全天候访问",
          description:
            "将您的行李一次性存放好，全天候随时取用。无论何时需要，您的物品都安全且易于取用。",
        },
        {
          title: "无忧预订",
          description:
            "体验我们简化的预订流程！简单步骤，快速确认，无与伦比的价格，尽在一个平台。",
        },
        {
          title: "全天候客户支持",
          description:
            "我们敬业的团队全天候为您提供帮助。有问题吗？我们的聊天机器人和客户服务随时准备提供帮助！",
        },
      ],
    },
    footer: {
      company: "公司",
      about: "关于",
      services: "服务",

      blog: "博客",
      support: "支持",
      faq: "常见问题",
      privacyPolicy: "隐私政策",
      terms: "条款和条件",

      contactInfo: "联系信息",
      address: "地址: 1 William St, Hawthorn, Victoria 3122, Australia",
      email: "邮箱: admin@urloker.com",
      phone: "电话: +01 234 567 89",
      followUs: "关注我们",
      companyDescription:
        "Urloker 为您的家和办公室提供安全便捷的行李存储解决方案。",
      copyright: "{year} Urloker. 设计由 EasytechInnovation",
    },
    luggageStorageLocations: {
      title: "我们推荐的行李寄存地点",
      searchButton: "搜索地点",
    },
    blogSection: {
      title: "旅游景点",
      subtitle: "探索我们的目的地",
      posts: [
        {
          title: "高质量的干净家具和工具为您的舒适",
          author: "管理员",
          comments: 20,
          views: 11,
          imageUrl: "furniture",
          excerpt:
            "了解我们的干净家具和工具如何提升您的旅行体验。了解为什么我们受到全球旅行者的信任。",
          detailsUrl: "blog-details.html",
        },
        {
          title: "我们致力于为社会事业提供医院行李服务",
          author: "管理员",
          comments: 21,
          views: 15,
          imageUrl: "hospital",
          excerpt:
            "阅读我们在提供专门行李给医院的计划。看看我们如何通过服务贡献于社会事业。",
          detailsUrl: "blog-details.html",
        },
        {
          title: "行李服务的卓越表现 - 我们为您服务",
          author: "管理员",
          comments: 22,
          views: 20,
          imageUrl: "luggage",
          excerpt:
            "了解更多关于我们致力于提供最佳行李服务的信息。我们优先考虑您的需求，确保顺畅的旅行体验。",
          detailsUrl: "blog-details.html",
        },
      ],
    },
    reviewSection: {
      title: "客户评价最好",
      reviews: [
        {
          name: "梅森·多",
          position: "客户",
          text: "这家公司提供的行李服务非常出色。他们对细节的关注和对质量的承诺极大地提高了我们的旅行体验。强烈推荐！",
          rating: 5,
        },
        {
          name: "艾莉莎·德克罗斯",
          position: "客户",
          text: "我对行李服务印象深刻。从快速送达到行李的完好无损，一切都很完美。他们的专业精神无可比拟。",
          rating: 5,
        },
        {
          name: "杰克·洛里",
          position: "客户",
          text: "感谢您提供如此出色的客户体验。行李箱做工精良，团队的支持非常宝贵。我肯定会成为回头客。",
          rating: 5,
        },
        {
          name: "杰克逊",
          position: "客户",
          text: "优秀的服务！团队超越了客户的期望，确保客户满意。行李质量一流，他们的支持随时待命。",
          rating: 5,
        },
        {
          name: "罗伯特",
          position: "客户",
          text: "服务非常好！团队超越了客户的期望，确保客户满意。行李质量一流，他们的支持随时待命。",
          rating: 5,
        },
      ],
    },
    howItWorks: {
      title: "如何运作：快速指南",
      steps: {
        book: {
          title: "预订",
          description: "找到您想要的位置，并以无与伦比的价格轻松预订。",
        },
        lock: {
          title: "锁定",
          description: "出示您的预订确认，并将您的行李安全地留在我们这里。",
        },
        enjoy: {
          title: "享受",
          description: "自由探索，当准备好时，无需任何麻烦即可取回您的物品。",
        },
      },
      video: {
        title: "操作指南视频",
        altText: "指南",
      },
    },
    about: {
      title: "关于我们",
      info: [
        `在 Urloker, 我们提供澳大利亚最好的行李寄存解决方案, 为旅行者提供无缝、安全且实惠的体验。无论您是在探索新城市还是需要一个安全的地方来存放您的物品, 我们轻松的预订流程、无与伦比的价格以及不断增长的合作地点网络让您可以轻松放心地存放行李。`,
        `客户可以直接通过我们的网站或在我们的任何合作商店扫描二维码预订寄存服务，从而获得最大的灵活性和便利性。我们先进的安全措施（包括监控系统）可确保您的行李安全无虞，并且随时可以取用。我们的“不着急取回”政策让您可以自由地寄存行李并在方便时取回——没有时间限制，没有压力，您可以按照自己的节奏自由探索。`,
        `Urloker 致力于提供最佳客户服务。我们专用的聊天机器人全天候待命，可快速高效地帮助解决任何客户服务问题，确保从始至终的顺畅体验。`,
        `作为澳大利亚领先的行李寄存服务提供商，我们的口号是“旅途自由”，体现了我们的使命：让每一位顾客安心，让他们知道自己的物品在安全的手中，同时享受探索城市的自由。`,
      ],
    },
    demo: {
      title: "Hassle Free Luggage Storage, Book in Seconds",
      list: [
        {
          title: "Security Guaranteed",
          desc: "$1000 protection",
        },
        {
          title: "Size Doesn't Matter",
          desc: "Flat Rate for All Bag Size No Need to Pay Extra",
        },
        {
          title: "No Hourly Fees",
          desc: "Enjoy full Day Flexibility with a Fixed Price Unbeatable Prices",
        },
        {
          title: "24/7 Support",
          desc: "We are Always Here for You",
        },
      ],
      end: "Enjoy Freedom in Every Journey with Urloker",
    },
  },
};

export default translations;
